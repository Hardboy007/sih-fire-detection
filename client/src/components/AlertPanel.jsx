...
const filters = ['ALL', 'Wildfire', 'Industrial Fire', 'Persistent Thermal Source', 'Low Risk']
...
function AlertPanel({ onHotspotSelect, selectedHotspot }) {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{ padding: '20px 16px 14px', borderBottom: '1px solid #1e2d3d' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
          <span style={{ fontSize: '15px', fontWeight: '600' }}>Fire Alert System</span>
        ...
        <div style={{ fontSize: '11px', color: '#4a6080' }}>NASA FIRMS — Live Data</div>
      </div>

      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1px', background: '#1e2d3d', borderBottom: '1px solid #1e2d3d' }}>
        {[
          { label: 'CRITICAL', value: critical, color: '#ef4444' },
          { label: 'THERMAL', value: thermal, color: '#eab308' },
          { label: 'LOW RISK', value: low, color: '#22c55e' },
        ].map(stat => (
          <div key={stat.label} style={{ background: '#0f1623', padding: '14px 8px', textAlign: 'center' }}>
            <div style={{ fontSize: '26px', fontWeight: '700', color: stat.color, lineHeight: 1 }}>{stat.value}</div>
            <div style={{ fontSize: '9px', color: '#4a6080', letterSpacing: '1px', marginTop: '4px' }}>{stat.label}</div>
          </div>
        ))}
      </div>

      {/* Filter Tabs */}
      <div style={{ padding: '10px 12px', borderBottom: '1px solid #1e2d3d', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
        {filters.map(f => (
          <button
            key={f}
            ...
            style={{
              ...
              fontWeight: '600',
              letterSpacing: '0.5px',
              background: activeFilter === f ? (f === 'ALL' ? '#38bdf8' : getColor(f)) : '#1e2d3d',
              color: activeFilter === f ? '#000' : '#4a6080',
              transition: 'all 0.15s'
            }}
          >
            {f === 'ALL' ? 'ALL' : f === 'Persistent Thermal Source' ? 'THERMAL' : f.toUpperCase()}
          </button>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px 12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', color: '#4a6080', fontSize: '12px', marginTop: '20px' }}>
            No incidents found
          </div>
        )}
      </div>
      ...
      {/* Legend */}
      <div style={{ padding: '12px 16px', borderTop: '1px solid #1e2d3d', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px' }}>
        {[
      ...
      {/* Bottom bar */}
      <div style={{ padding: '8px 16px', borderTop: '1px solid #1e2d3d', fontSize: '9px', color: '#2a4060', letterSpacing: '1.5px', textAlign: 'center' }}>
